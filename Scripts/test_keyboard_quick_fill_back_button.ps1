...
$checks = [ordered]@{
    ...
    "quick fill back icon uses dedicated 28pt size" = $source.Contains("private static let quickFillBackIconPointSize: CGFloat = 28") -and $source.Contains('button.setImage(keyboardIcon(.back, fallbackSystemName: "chevron.left", pointSize: Self.quickFillBackIconPointSize), for: .normal)')
    ...
    "quick fill scroll view starts below fixed top bar" = $source.Contains("candidatePageScrollTopToQuickFillTopBarConstraint = candidatePageScrollView.topAnchor.constraint(equalTo: quickFillTopBar.bottomAnchor, constant: 4)") -and $source.Contains("candidatePageScrollTopToQuickFillTopBarConstraint?.isActive = visible")
