...
def main() -> int:
    ...
    checks = {
        ...
        "candidate refresh keeps visible candidates while pending": "applyCandidateRefreshResult([], mode: queryMode" not in source,
        "candidate refresh tracks pending state": "private var isCandidateRefreshPending = false" in source,
        "pending candidate refresh disables stale candidate taps": "updateCandidateControlsEnabled()" in source,
        "pending candidate refresh begins before background lookup": "isCandidateRefreshPending = true" in source,
        "candidate interaction resumes after latest results": "isCandidateRefreshPending = false" in source,
        "candidate buttons keep appearance while blocking pending taps": "button.isUserInteractionEnabled = !isCandidateRefreshPending" in source,
        "candidate expand button keeps appearance while blocking pending taps": "candidateExpandButton.isUserInteractionEnabled = !isCandidateRefreshPending" in source,
